...
RWStructuredBuffer<Particle> g_particle : register(u0);         // 기본적인 배열
RWStructuredBuffer<ComputeShared> g_shared : register(u1);      // 단지 하나

// CS_Main
// g_vec2_1 : DeltaTime / AccTime(누적시간)
// g_int_0  : Particle Max Count
// g_int_1  : AddCount
// g_vec4_0 : MinLifeTime / MaxLifeTime / MinSpeed / MaxSpeed
// 병렬 방식은 순서가 보장이 안 되는 것을 염두하고 짜야된다.
[numthreads(1024, 1, 1)]
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
    ...
    GroupMemoryBarrierWithGroupSync();  // 모든 스레드들가 위의 코드가 실행될 때까지 기다린다.
